...
type codifiedLine = {
    sinal_level: number;    // Alturas diferentes que o sinal pode assumir
    sinal_variation: number;// Quantas trocas um sinal faz para representar um dado
    data: string[];         // Dados codificados
    signal: number[][];     // Os sinais em si
    base_line: number;      // Linha referência
    start_level: number;    // Nivel de sinal inicial
    data_per_line: number; // Quantos dados serão codificados por linha
...
function generateView(CL: codifiedLine, div_output: HTMLElement): void {
...
function encode8b6t(data: String): codifiedLine {
    ...
    for (let index = 0; index < data.length / 8; index++) {
        const data_slice: string = data.slice(index * 8, index * 8 + 8);

        CL.data.push(data_slice);

        var value = parseInt(data_slice, 2);

        var symbol: string = json8bt6.data[value].symbol;
        var value_symbol = json8bt6.data[value].value;
       
            if (value_symbol == 0) {
                let signal = [];
                for (let index_s = 0; index_s < symbol.length; index_s++) {
                    

                    switch (symbol[index_s]) {
                        case '+':
                            signal.push(2);
                            break;
                        case '0':
...
function encodeNRZ_L(data: String): codifiedLine {
    let CL: codifiedLine = <codifiedLine>{};
    CL.data = [];
    CL.signal = [];
    CL.base_line = 1;
...
function encodeNRZ_I(data: String): codifiedLine {
    let CL: codifiedLine = <codifiedLine>{};
    CL.data = [];
    CL.signal = [];
    CL.base_line = 1;
    ...
    var prev_level: number = CL.start_level;
